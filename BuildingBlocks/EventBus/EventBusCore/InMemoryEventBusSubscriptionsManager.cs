...
namespace DistributedDPX.BuildingBlocks.EventBus.EventBusCore
{
	public class InMemoryEventBusSubscriptionsManager : IEventBusSubscriptionsManager
	{
		...
		{
			_handlers = new Dictionary<string, List<EventBusSubscriptionInfo>>();
		...
		private void DoAddSubscription(Type handlerType, string eventName, bool isDynamic)
		{
			...
			if (isDynamic)
			{
				_handlers[eventName].Add(EventBusSubscriptionInfo.Dynamic(handlerType));
			}
			else
			{
		...
		private void DoRemoveHandler(string eventName, EventBusSubscriptionInfo subsToRemove)
		{
		...
		private EventBusSubscriptionInfo FindDynamicSubscriptionToRemove<TH>(string eventName)
			where TH : IDynamicIntegrationEventHandler
		{
		...
		private EventBusSubscriptionInfo FindSubscriptionToRemove<T, TH>()
			 where T : IntegrationEvent
			 where TH : IIntegrationEventHandler<T>
		{
			var eventName = GetEventKey<T>();
		...
		private EventBusSubscriptionInfo DoFindSubscriptionToRemove(string eventName, Type handlerType)
		{
			if (!HasSubscriptionsForEvent(eventName))
			{
				return null;
